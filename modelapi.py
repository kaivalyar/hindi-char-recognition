...
def preprocess(img):
    ...
    #img = cv2.imread(os.path.join('','temp.png'),0)
...
def predict(img):
    ...
    matra_model = load_model('matra_model')
    dot_model = load_model('dot_model')
    ...
    #total_pred = [i for i in total_pred if i!=0]
    print("Integers" + str(total_pred))
    print("Hex" + str([hex(i) for i in total_pred]))
    return total_pred

im = cv2.imread('ok.png')
predict(im)
